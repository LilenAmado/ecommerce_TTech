...
const ProductDetail = () => {
  ...
  const { cart, setCart, setIsCartEmpty, setIsHovered} = useContext(ProyectContext);
    ...
    const handleAddToCart = (prod) => {
        ...
        addOrUpdateCart(normalizedProduct, cart, setCart, setIsCartEmpty);
        setIsHovered(true);
    };
    ...
    return (
        <div className="product-detail-container">
            <div>
                <img src={product.img} alt={product.name} width={300} className='img-product-detail'/>
            </div>
            <div className='product-detail-info'>
                ...
                <Button text={"Agregar al carrito"} onClick={() => handleAddToCart(product)} />
            </div>
        </div>
    );
};
